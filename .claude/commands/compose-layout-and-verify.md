...
Implement the design using the `compose-layout` skill, passing $1 as the figma link and $2 as the target repo. Let the skill run to completion (inventory confirmation, write, etc.).

If `compose-layout` halts (missing library, ambiguous inputs, user declines the inventory), do not proceed to Step 2 — report the stop reason and exit.

## Step 2 — Verify

Follow `.claude/commands/verify-design.md` with **no positional args**. Its "Resolving inputs" section will auto-detect the just-composed run in this conversation (page component path, Figma link, and `$APERIA_DS_PACKAGE` reused from `compose-layout`'s Phase 1), prompt the user once to confirm via `AskUserQuestion`, and then delegate to the `design-verifier` subagent.
